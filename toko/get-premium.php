...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    try {
        ...
        $queryTokoPremium = "
            ...
            WHERE p.nama = 'premium' AND l.tanggal_berakhir > CURDATE()"; // Filter untuk paket "Premium"
        
        $stmtTokoPremium = $pdo->query($queryTokoPremium);
        $tokoPremium = $stmtTokoPremium->fetchAll(PDO::FETCH_ASSOC);

        // Format URL gambar logo toko
        foreach ($tokoPremium as &$toko) {
            if (!empty($toko['logo'])) {
        ...
        echo json_encode([
            'success' => true,
            'data' => [
                'premium_stores' => $tokoPremium // Menampilkan semua toko premium dengan paket "Premium"
            ]
        ]);
